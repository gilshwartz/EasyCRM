...
foreach ($results['requests'] as $request) {
    ...
    $line['fullname'] =  $request['Contact']['firstname'] . ' ' . $request['Contact']['lastname'];
    $line['company'] =  $request['Contact']['company_name'];
    $line['country'] =  $results['countries'][$request['Contact']['country']];
    $csv->addRow($line);
}
